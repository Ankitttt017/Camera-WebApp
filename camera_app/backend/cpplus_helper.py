    ...
    with get_db_connection() as connection:
        ...
        for column_name, column_type in {
            ...
            if column_name not in existing_columns:
                try:
                    cursor.execute(f"ALTER TABLE recordings ADD COLUMN {column_name} {column_type}")
        ...
        if exists:
            ...
            cursor = cursor.execute(
                ...
                UPDATE recordings SET
                    ...
                    transcript_error = COALESCE(:transcript_error, transcript_error),
...
def prune_missing_recordings(storage_root: str | Path) -> int:
    ...
    with get_db_connection() as connection:
        ...
        if missing_paths:
            cursor.executemany("DELETE FROM recordings WHERE file_path = ?", [(path,) for path in missing_paths])
...
def recording_stats(request: RecordingIndexRequest) -> dict:
    ...
    def scalar(connection: sqlite3.Connection, query: str, query_params: dict[str, str | int] | None = None):
        value = cursor.execute(query, query_params or {}).fetchone()[0]
        return value or 0
